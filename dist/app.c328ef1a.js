...
function newsFeed() {
  ...
  template = template.replace('{{__next_page__}}', store.currentPage + 1); // join = 배열의 요소들을 컴마로 구분해 하나의 문자열로 만들어줌 / 파라미터 = 구분자 ('') => 구분자 없이 합치겠다는 말
  // (innerHTML은 문자열만 넣을 수 있기 때문에 변환하는 작업 필요)  
...
function router() {
  var routePath = location.hash; // # 이여도 빈 문자열로 읽어들임
